...
void rpc_hardware_serial1_uart_master::_flush()
{
    for (int i = Serial1.available(); i > 0; i--) Serial1.read();
}

bool rpc_hardware_serial1_uart_master::get_bytes(uint8_t *buff, size_t size, unsigned long timeout)
{
    Serial1.setTimeout(timeout + 1);
    bool ok = (Serial1.readBytes((char *) buff, size) == size) && (!_same(buff, size));
    if (!ok) delay(_get_short_timeout);
    return ok;
}

bool rpc_hardware_serial1_uart_master::put_bytes(uint8_t *buff, size_t size, unsigned long timeout)
{
    (void) timeout;
    return Serial1.write((char *) buff, size) == size;
}

void rpc_hardware_serial1_uart_slave::_flush()
{
    for (int i = Serial1.available(); i > 0; i--) Serial1.read();
}

bool rpc_hardware_serial1_uart_slave::get_bytes(uint8_t *buff, size_t size, unsigned long timeout)
{
    Serial1.setTimeout(timeout + 1);
    return Serial1.readBytes((char *) buff, size) == size;
}

bool rpc_hardware_serial1_uart_slave::put_bytes(uint8_t *buff, size_t size, unsigned long timeout)
{
    (void) timeout;
    return Serial1.write((char *) buff, size) == size;
}
